def call(Map params) {
    pipeline {
        parameters {
            ...
            string description: 'Docker context', name: 'DOCKER_CONTEXT', trim: true, defaultValue: params.DOCKER_CONTEXT ?: '.'
        }    
            ...
            stage('Dockerize') {
                steps {
                    script {
                        ...
                        def image = docker.build("${env.REGISTRY_URL}/${params.REGISTRY_CATEGORY_NAME}/${params.PROJECT_NAME}:${env.BUILD_ID}", "-f ${params.DOCKERFILE_PATH} ${params.DOCKER_CONTEXT}")
                    }
                }
            }
